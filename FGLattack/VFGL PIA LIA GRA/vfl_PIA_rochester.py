...
with open('./{}_feats.pkl'.format('rochester'), 'rb') as f1:
    feature = pkl.load(f1)
    feature = torch.from_numpy(feature.toarray())
with open('./{}_adj.pkl'.format('rochester'), 'rb') as f2:
    adj = pkl.load(f2, encoding='latin1')
    adj = torch.from_numpy(adj.todense()).long()
label = np.load('./{}_labels.npy'.format('rochester'))

"""rochester数据集 主任务3分类，隐私属性为教育水平"""
utility_label = torch.LongTensor(label[:, 0]).cuda()
privacy_property = torch.LongTensor(label[:, 5]).cuda()
...
print(privacy_property.shape)
idx_train = range(3000)
idx_test = range(3000,privacy_property.shape[0])
...
class SplitNN(nn.Module):
    ...
    def forward(self, x, stage, epoch):
        for i in range(len(x)):
            ...
            # print(stage)
            # if i == 1 and stage=='train' and epoch==49:
            #
            ...
            if epoch == 49 and stage=='train':
                torch.save(self.output[1], 'rochester_embedding_train.pth')
...
def create_models(partition, input_size, hidden_sizes, output_size):
    ...
    for _ in range(1, partition):
        models.append(GCN(nfeat=118, nhid=128, nclass=hidden_sizes[1]).cuda())
...
def adversary():
    ...
    adv_train = torch.load('rochester_embedding_train.pth').cuda()
    adv_test = torch.load('rochester_embedding_train.pth').cuda()
    # adv_train = 1000*torch.randn(adv_train.shape).cuda() +adv_train
    # adv_test = 1000*torch.randn(adv_test.shape).cuda()+adv_test
    ...
    # 训练攻击者模型
    # optim_ = optim.SGD(AttackModel.parameters(), lr=0.01, momentum=0.9)
    ...
    _, acc, f1 = adversary_class_test(optim_, AttackModel, adv_test, test_privacy_property)
    print('the attack test epoch {} and acc is {}, f1 is {}.'.format(i, acc, f1))
